...
function ContactCard({ candidate }) {
  ...
  return (
    <div className="rounded-2xl bg-[#FAF6EE] p-3">
      <p className="text-[13px] font-bold text-[#3B332A]">{candidate.name}</p>
      <p dir="ltr" className="mt-0.5 flex items-center gap-1 text-[12px] text-[#8B8175]">
        <Phone size={12} /> {candidate.phone}
      </p>
      <button
        onClick={handleCopy}
        className="mt-2 flex w-full items-center justify-center gap-1 rounded-xl border border-[#E7DECD] bg-white py-1.5 text-[11px] font-semibold text-[#6F4A2E] transition active:scale-95 hover:bg-[#FAF6EE]"
      >
      ...
      {candidate.referenceContacts && (
        <div className="mt-2 rounded-xl border-2 border-[#6F4A2E] bg-white p-2">
          <p className="mb-1 text-[10px] font-bold text-[#6F4A2E]">מספרים לבירורים</p>
          <p className="mb-1.5 whitespace-pre-wrap text-[11px] text-[#3B332A]">{candidate.referenceContacts}</p>
          <button
            onClick={handleCopyReferenceContacts}
            className="flex w-full items-center justify-center gap-1 rounded-lg bg-[#6F4A2E] py-1.5 text-[11px] font-semibold text-white transition active:scale-95"
          >
  ...
  return (
    <div className="rounded-3xl border border-[#E7DECD] bg-white p-4 shadow-[0_4px_18px_rgba(58,51,53,0.06)]">
      <div className="flex items-center justify-between">
        <h3 className="text-[15px] font-bold text-[#3B332A]">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => setOpen((v) => !v)} className="rounded-full p-1.5 hover:bg-[#FAF6EE]" aria-label="פתיחת יומן">
      ...
      <div className="mt-2 flex items-center justify-between">
        {proposal.assignee ? (
          <span className="flex items-center gap-1.5 rounded-full bg-[#F3E7D5] px-2.5 py-1 text-[11px] font-bold text-[#4E3220]">
            <UserCheck size={12} /> מטופל/ת ע״י {proposal.assignee}
          </span>
        ) : (
          <span className="text-[11px] font-semibold text-[#B4AA9C]">טרם שויך לאיש צוות</span>
        )}
        {proposal.assignee ? (
          <button
            onClick={() => assignProposal(proposal.id, null)}
            className="flex items-center gap-1 text-[11px] font-semibold text-[#B4AA9C] hover:text-[#6F4A2E]"
          >
          ...
          <button
            ...
            className="rounded-full bg-[#6F4A2E] px-3 py-1 text-[11px] font-bold text-white transition active:scale-95"
          >
            לקחתי על עצמי
          </button>
        )}
      ...
      <div className="mt-4 rounded-2xl bg-[#FDF6E3] p-3">
        <p className="mb-1 flex items-center gap-1 text-[11px] font-bold text-[#8A6A12]">
        ...
        <textarea
          ...
          className="w-full resize-none rounded-xl border-none bg-transparent text-[12px] text-[#3B332A] outline-none placeholder:text-[#B4AA9C]"
        />
      </div>
      ...
      {open && (
        <div className="mt-4 space-y-3 border-t border-[#E7DECD] pt-3">
          <div>
            <p className="mb-1.5 text-[12px] font-semibold text-[#3B332A]">עדכון סטטוס</p>
          ...
          <div>
            <input
              ...
              className="w-full rounded-xl border border-[#E7DECD] bg-white px-3 py-2 text-[13px] outline-none focus:border-[#6F4A2E]"
            />
          </div>
          ...
          <div>
            ...
            <ul className="space-y-1.5">
              {[...proposal.journal].reverse().map((entry) => (
                <li key={entry.id} className="rounded-xl bg-[#FAF6EE] px-3 py-2 text-[12px]">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-[#3B332A]">{entry.status}</span>
                    <span className="text-[#B4AA9C]">{new Date(entry.date).toLocaleDateString("he-IL")}</span>
                  </div>
                  {entry.note && <p className="mt-0.5 text-[#8B8175]">{entry.note}</p>}
                  <p className="mt-0.5 text-[10px] text-[#B4AA9C]">עודכן ע״י {entry.author}</p>
                </li>
              ))}
            </ul>
          </div>
