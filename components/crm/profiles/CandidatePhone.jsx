...
// שני אזורי פנייה נפרדים בכרטיס המועמד/ת, זה מתחת לזה:
//
//   1. פנייה ישירה למועמד/ת - חיוג, וואטסאפ ו-SMS. גלויה לכל אנשי הצוות
//      והשגרירים ללא יוצא מן הכלל, ואינה תלויה בשיוך שגריר/ה. זה מה
//      שמאפשר לכל אחד/ת לפנות ולהציע הצעה בלי להיחסם.
//
//   2. איש/אשת הקשר בצוות - השגריר/ה שמכיר/ה את המועמד/ת אישית. זו
//      הכתובת לשאלות רוחב ולהתייעצות, ולא שומר/ת סף.
//
// שני האזורים עומדים בזה אחר זה ואף אחד מהם אינו מבטל את השני.
//
...
export default function CandidatePhone({ candidate, compact = false }) {
  ...
  // אזור 1: פנייה ישירה למועמד/ת. תמיד ראשון, כי זו הפעולה השכיחה.
  const direct = candidate?.phone ? (
