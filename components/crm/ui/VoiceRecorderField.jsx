...
export default function VoiceRecorderField({ value, onChange, label = "הקלטה קצרה (לא חובה)" }) {
  ...
  return (
    <div>
      ...
      {value ? (
        <div className="flex items-center gap-2">
          {url ? (
            ...
            <p className="flex-1 text-[12px] text-[#8A8285]">טוען הקלטה...</p>
          )}
          <button
            type="button"
            onClick={() => onChange(null)}
            aria-label="מחיקת ההקלטה"
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[#C24545] transition active:scale-90 hover:bg-red-50"
          >
            <Trash2 size={15} />
          </button>
        </div>
        ...
        <button
          ...
          className={`flex w-full items-center justify-center gap-1.5 rounded-2xl py-2.5 text-[13px] font-semibold text-white transition active:scale-[0.98] disabled:opacity-60 ${
            recording ? "bg-red-500" : "bg-[#8C4A55]"
          }`}
        >
      ...
      {error && <p className="mt-1 text-[11px] text-[#C24545]">{error}</p>}
    </div>
  );
}
