...
export default function MatchesPage() {
  ...
  if (quizCompleted) {
    ...
    return (
      <div className="px-4 py-4">
        ...
        <div className="mt-4 rounded-3xl bg-gradient-to-l from-[#C06E5E] to-[#A05243] p-5 text-white shadow-lg">
          ...
          <div className="mt-16 text-center text-sm leading-relaxed text-[#8C7B6B]">
            ...
            {ranked.below.length > 0 && (
              <button
                ...
                className="mt-3 rounded-xl bg-[#C06E5E] px-3.5 py-2 text-[12px] font-semibold text-white"
              >
              ...
              <div key={r.candidate.id} className="relative">
                <span
                  className={`absolute -top-2 right-4 z-10 rounded-full px-2.5 py-1 text-[11px] font-bold text-white shadow ${
                    r.score >= MATCH_THRESHOLD ? "bg-[#8C9A78]" : "bg-[#C9A063]"
                  }`}
                >
                  {r.score}% התאמה
                </span>
                <ProfileCard candidate={r.candidate} />
                <p className="mt-1 px-1 text-[11px] leading-relaxed text-[#8C7B6B]">
  ...
  return (
    <div className="px-4 py-6">
      ...
      {!showWizard ? (
        <div className="mt-8 flex flex-col items-center rounded-3xl border border-[#EADCCB] bg-white p-6 text-center shadow-[0_4px_18px_rgba(58,51,53,0.06)]">
          <div className="mb-3 flex h-16 w-16 items-center justify-center rounded-full bg-[#F7DFD8]">
            <Sparkles size={28} className="text-[#C06E5E]" />
          </div>
          <h2 className="text-lg font-bold text-[#5A4A3C]">רוצה לקבל התאמות מותאמות אישית?</h2>
          <p className="mt-2 max-w-xs text-[13px] leading-relaxed text-[#8C7B6B]">
