...
import { REGIONS, religiousLevelsFor, EDUCATION_OPTIONS, YESHIVA_LEVELS, OCCUPATION_OPTIONS, smokingOptionsFor, TRAITS } from "@/lib/crm/mockData";
  ...
  return (
    <div className="px-4 py-6">
      ...
      <div className="mt-4 space-y-4">
        ...
        {role === "admin" && (
          <label className="flex items-center gap-2.5 rounded-2xl border-2 border-[#C4584C] bg-red-50 px-4 py-3">
            <input
              type="checkbox"
              checked={form.confidential}
              onChange={(e) => set({ confidential: e.target.checked })}
              className="h-5 w-5 shrink-0 accent-[#C4584C]"
            />
            <span className="text-[13px] font-bold text-[#C4584C]">כרטיס חסוי (גלוי למנהלת בלבד)</span>
          </label>
        )}
            ...
            {pdfUrl ? (
              <div className="space-y-1.5">
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-1 rounded-xl border border-red-200 bg-red-50 py-1.5 text-[11px] font-semibold text-[#C4584C]"
                >
