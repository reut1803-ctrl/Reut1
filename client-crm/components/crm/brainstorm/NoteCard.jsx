...
export default function NoteCard({
  ...
  return (
    ...
    >
      {gold && !isReply && (
        <span className="absolute -top-2 right-4 rounded-full bg-[#C9A063] px-2 py-0.5 text-[9px] font-bold text-white shadow-sm">
          הכי מהדהד בצוות
        </span>
      )}
      ...
      <div className="mt-3 flex items-center justify-between gap-2">
        ...
        {canDelete && (
          <button
            ...
            aria-label="מחיקה"
            className="shrink-0 rounded-full p-1.5 text-[#C4584C] transition hover:bg-white/60"
          >
            <Trash2 size={14} />
          </button>
        )}
