...
export default function CopyStaffButton({ name, phone, className = "" }) {
  ...
  return (
    <button
      ...
      className={`shrink-0 rounded-full p-1.5 text-[#2E8BA8] transition active:scale-90 hover:bg-white/70 ${className}`}
    >
      {copied ? <Check size={14} className="text-[#21867F]" /> : <Copy size={14} />}
    </button>
  );
}
