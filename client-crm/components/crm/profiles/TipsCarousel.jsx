    ...
    <div className="mb-4 rounded-2xl border border-[#F0E3C0] bg-[#FDF6EC] p-3.5">
      <div className="mb-2 flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <p className="text-[11px] font-bold uppercase tracking-wide text-[#8A6A32]">טיפ בשידוכים</p>
        </div>
        {isAdmin && (
          <button
            onClick={() => setShowAdd((v) => !v)}
            aria-label="הוספת טיפ"
            className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#8A6A32] text-white transition active:scale-90"
          >
            <Plus size={14} />
          </button>
        )}
      ...
      {showAdd && (
        <div className="mb-3 flex gap-1.5">
          ...
          <button
            ...
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-[#8A6A32] text-white"
          >
            <Check size={14} />
          </button>
        </div>
