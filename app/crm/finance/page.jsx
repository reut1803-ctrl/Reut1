...
function ProofUpload({ url, onUpload, label }) {
  ...
  if (url) {
    return (
      <a href={url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[11px] font-semibold text-[#4A6552]">
        <ImageIcon size={12} /> צפייה באסמכתא
      </a>
    );
  }
  return (
    <label className="flex cursor-pointer items-center gap-1 text-[11px] font-semibold text-[#844442]">
...
export default function FinancePage() {
  ...
  return (
    <div className="px-4 py-6">
      ...
      <p className="mt-1 text-[13px] text-[#7C6E60]">אזור זה נגיש למנהלת בלבד ומוסתר לחלוטין מהצוות</p>

      <h2 className="mt-6 mb-3 text-[15px] font-bold text-[#3A2E26]">מחירון שירותים</h2>
      <div className="rounded-3xl border border-[#CCBDAB] bg-white p-4 shadow-[0_4px_18px_rgba(58,51,53,0.06)]">
        <div className="space-y-2">
          {serviceTypes.map((s) => (
            <div key={s.id} className="flex items-center justify-between rounded-xl bg-[#E8DCCB] px-3 py-2">
              <div>
                <p className="text-[13px] font-semibold text-[#3A2E26]">{s.name}</p>
      ...
      <div className="mt-8 mb-3 flex items-center justify-between">
        <h2 className="text-[15px] font-bold text-[#3A2E26]">חיובים ועמלות ({filteredCharges.length})</h2>
      </div>
        ...
        {[
          ...
          <button
            ...
            className={`flex-1 rounded-xl py-2 text-[12px] font-bold transition ${
              filter === f.key ? "bg-[#F0E2DE] text-[#5E2F2D]" : "text-[#7C6E60]"
            }`}
          >
            {f.label}
          </button>
      ...
      {filteredCharges.length === 0 ? (
        <p className="py-6 text-center text-sm text-[#7C6E60]">אין חיובים להצגה</p>
      ) : (
        <div className="space-y-3">
          {filteredCharges.map((c) => (
            <div key={c.id} className="rounded-3xl border border-[#CCBDAB] bg-white p-4 shadow-[0_4px_18px_rgba(58,51,53,0.06)]">
              <div className="flex items-center justify-between">
                <p className="text-sm font-bold text-[#3A2E26]">{candidateName(c.candidateId)}</p>
                <span className="text-[11px] text-[#A2937F]">{new Date(c.createdAt).toLocaleDateString("he-IL")}</span>
              </div>
              <p className="text-[12px] text-[#7C6E60]">{c.serviceName}</p>

              <div className="mt-3 grid grid-cols-2 gap-2">
                <div className="rounded-2xl bg-[#E8DCCB] p-3">
                  <p className="text-[11px] font-semibold text-[#3A2E26]">תשלום מועמד/ת</p>
                  <p className="mt-0.5 text-sm font-bold text-[#3A2E26]">₪{c.price}</p>
                  <button
                    onClick={() =>
                    ...
                    className={`mt-1.5 flex w-full items-center justify-center gap-1 rounded-full px-2 py-1 text-[11px] font-bold transition ${
                      c.candidatePaymentStatus === "שולם" ? "bg-[#DDE6DF] text-[#4A6552]" : "bg-[#EDE2D6] text-[#C24545]"
                    }`}
                  >
                ...
                <div className="rounded-2xl bg-[#E8DCCB] p-3">
                  <p className="text-[11px] font-semibold text-[#3A2E26]">עמלת {staffName(c.staffId)}</p>
                  <p className="mt-0.5 text-sm font-bold text-[#3A2E26]">₪{c.commission}</p>
                  <button
                    onClick={() => updateChargeStaffPayout(c.id, c.staffPayoutStatus === "שולם" ? PAYMENT_STATUSES[0] : PAYMENT_STATUSES[1])}
                    className={`mt-1.5 flex w-full items-center justify-center gap-1 rounded-full px-2 py-1 text-[11px] font-bold transition ${
                      c.staffPayoutStatus === "שולם" ? "bg-[#DDE6DF] text-[#4A6552]" : "bg-[#EDE2D6] text-[#C24545]"
                    }`}
                  >
