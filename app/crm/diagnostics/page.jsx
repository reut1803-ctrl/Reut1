...
export default function DiagnosticsPage() {
  ...
  return (
    <div className="px-4 py-6">
      ...
      <p className="mt-1 text-[13px] text-[#8A8285]">
      ...
      <div className="mt-5 space-y-3">
        {steps.map((s, i) => (
          <div
            key={i}
            className={`rounded-2xl border-2 p-3 ${s.ok ? "border-[#20A66B] bg-[#20A66B]/5" : "border-[#C24545] bg-red-50"}`}
          >
            <p className={`flex items-center gap-1.5 text-[13px] font-bold ${s.ok ? "text-[#178A57]" : "text-[#C24545]"}`}>
              {s.ok ? <Check size={15} /> : <X size={15} />} {i + 1}. {s.title}
            </p>
            {s.detail && <p className="mt-1 break-words text-[12px] text-[#3A3335]">{s.detail}</p>}
          </div>
        ))}
      </div>

      {!running && steps.length > 0 && (
        <p className="mt-5 rounded-2xl bg-[#F6F5F4] p-3 text-[12px] text-[#8A8285]">
