...
function DuplicateAlertDialog({ proposal, onContinue, onCancel }) {
  const whenText = proposal?.createdAt ? new Date(proposal.createdAt).toLocaleDateString("he-IL") : "";
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center px-6" role="alertdialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-xs rounded-3xl bg-white p-5 text-center shadow-2xl">
        <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-[#FFF8E7]">
          <AlertTriangle size={22} className="text-[#946200]" />
        </div>
        <p className="text-[13px] font-bold leading-relaxed text-[#3A3335]">
        ...
        {(whenText || proposal?.rationale || proposal?.isHistory) && (
          <div className="mt-3 rounded-2xl bg-[#F6F5F4] px-3 py-2 text-right">
            {whenText && (
            ...
            {proposal?.rationale && (
              <p className="mt-1 text-[11px] leading-relaxed text-[#8A8285]">הרציונל שנכתב אז: {proposal.rationale}</p>
            )}
          </div>
        )}
...
export default function ProposalsPage() {
  ...
  // הזוג הזה כבר עלה בעבר וירד מהפרק - בין אם במערכת ובין אם בהיסטוריה שהוזנה ידנית
  const previousDropped = droppedProposalFor(selection.male, selection.female);

  // חוסמים את יצירת ההצעה ומקפיצים התראה. ההצעה תיווצר רק אחרי אישור מפורש.
  const handleCreateClick = () => {
    if (previousDropped) setShowDuplicateAlert(true);
    else handleCreate();
  };
    ...
    <div className="px-4 py-6">
      ...
      {previousDropped && (
        <div className="mt-3 rounded-2xl border border-[#F0DFA0] bg-[#FFF8E7] p-3">
          ...
          <p className="mt-1 text-[11px] text-[#8A8285]">אפשר להמשיך ולהציע שוב, אבל המערכת תבקש אישור לפני ההקמה.</p>
        </div>
      )}
      ...
      <div className="mt-8">
        <h2 className="mb-3 text-[15px] font-bold text-[#3A3335]">הצעות פעילות ({activeProposals.length})</h2>
