  ...
  return (
    <div>
      <div className="mb-1 flex items-center justify-between text-[12px]">
        ...
        <span className="font-bold text-[#2E2116]">
          {value} / {goal}
        </span>
      </div>
      <div className="h-2 w-full overflow-hidden rounded-full bg-[#E9DCC4]">
...
export default function DashboardPage() {
  ...
  return (
    <div className="px-4 py-6">
      ...
      <p className="mt-1 text-[13px] text-[#7A6A55]">מעורבות צוות, נהלים, טיפים ומעקב מיילים</p>

      <h2 className="mt-6 mb-3 flex items-center gap-1.5 text-[15px] font-bold text-[#2E2116]">
      ...
      <div className="rounded-3xl border border-[#D9C6A5] bg-white p-4 shadow-[0_4px_18px_rgba(58,51,53,0.06)]">
        <div className="space-y-2">
          {authAllowlist.map((entry) => (
            <div key={entry.email} className="flex items-center justify-between rounded-xl bg-[#F2E8D5] px-3 py-2">
              <div>
                <p className="text-[13px] font-semibold text-[#2E2116]">{entry.name}</p>
                <p dir="ltr" className="text-left text-[11px] text-[#7A6A55]">
        ...
        <div className="mt-3 space-y-2 border-t border-[#D9C6A5] pt-3">
          <input
            ...
            className="w-full rounded-xl border border-[#D9C6A5] bg-white px-3 py-2 text-left text-sm outline-none focus:border-[#5B3418]"
          />
          <input
            type="text"
            value={newName}
      ...
      <Link
        href="/crm/finance"
        className="mb-6 flex items-center justify-between rounded-3xl border border-[#D9C6A5] bg-white p-4 shadow-[0_4px_18px_rgba(58,51,53,0.06)] transition active:scale-[0.99]"
      >
        <div className="flex gap-6">
          <div>
            <p className="text-[11px] text-[#7A6A55]">חובות מועמדים פתוחים</p>
            <p className="text-lg font-bold text-[#C24545]">₪{openCandidateDebt.toLocaleString()}</p>
          </div>
          <div>
            <p className="text-[11px] text-[#7A6A55]">עמלות ממתינות לצוות</p>
            <p className="text-lg font-bold text-[#B8860B]">₪{pendingStaffCommission.toLocaleString()}</p>
          </div>
        </div>
        <span className="text-[12px] font-semibold text-[#5B3418]">לניהול ←</span>
      </Link>

      <h2 className="mt-6 mb-3 flex items-center gap-1.5 text-[15px] font-bold text-[#2E2116]">
        <Target size={17} /> יעדי פעילות שבועיים
      </h2>
      <div className="mb-6 rounded-3xl border border-[#D9C6A5] bg-white p-4 shadow-[0_4px_18px_rgba(58,51,53,0.06)]">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <p className="mb-1 text-[12px] font-semibold text-[#2E2116]">צפיות בכרטיסים</p>
            <input
              type="number"
          ...
          <div>
            <p className="mb-1 text-[12px] font-semibold text-[#2E2116]">השמעות הקלטות</p>
            <input
              type="number"
      ...
      <h2 className="mt-6 mb-3 text-[15px] font-bold text-[#2E2116]">מעורבות צוות (השבוע)</h2>
      <div className="space-y-3">
        {staffList.map((s) => {
          const t = telemetry[s.email] || {};
          return (
            <div key={s.email} className="rounded-3xl border border-[#D9C6A5] bg-white p-4 shadow-[0_4px_18px_rgba(58,51,53,0.06)]">
              <p className="mb-3 text-sm font-bold text-[#2E2116]">{s.name}</p>
      ...
      <div className="rounded-3xl border border-[#D9C6A5] bg-white p-4 shadow-[0_4px_18px_rgba(58,51,53,0.06)]">
        <textarea
          ...
          rows={7}
          className="w-full resize-none rounded-xl border border-[#D9C6A5] bg-white px-3 py-2 text-[13px] leading-relaxed outline-none focus:border-[#5B3418]"
        />
          ...
          {staffList.map((s) => (
            <span
              ...
              className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${
                termsAccepted[s.email] ? "bg-[#E4F0CE] text-[#5E9128]" : "bg-[#F2E8D5] text-[#A08D74]"
              }`}
            >
      ...
      <div className="rounded-3xl border border-[#D9C6A5] bg-white p-4 shadow-[0_4px_18px_rgba(58,51,53,0.06)]">
        <div className="space-y-2">
          {tips.map((tip, i) => (
            <div key={i} className="flex items-start justify-between gap-2 rounded-xl bg-[#F2E8D5] px-3 py-2">
              <p className="text-[13px] leading-relaxed text-[#2E2116]">{tip}</p>
              <button onClick={() => removeTip(i)} aria-label="הסרת טיפ" className="shrink-0 rounded-full p-1 hover:bg-white">
                <Trash2 size={14} className="text-[#C24545]" />
              </button>
            </div>
          ))}
          {tips.length === 0 && <p className="text-center text-[12px] text-[#A08D74]">אין עדיין טיפים</p>}
        </div>
        <div className="mt-3 flex gap-2 border-t border-[#D9C6A5] pt-3">
          <input
            type="text"
            value={newTip}
      ...
      {emailLog.length === 0 ? (
        <p className="text-center text-sm text-[#7A6A55]">עדיין לא נשלחו מיילים</p>
      ) : (
        <div className="space-y-2">
          {emailLog.map((m) => (
            <div key={m.id} className="rounded-2xl border border-[#D9C6A5] bg-white p-3 text-[12px]">
              <div className="flex items-center justify-between">
                <span className="font-bold text-[#2E2116]">{m.subject}</span>
                <span className="text-[#A08D74]">{new Date(m.date).toLocaleString("he-IL")}</span>
              </div>
              <p className="mt-1 text-[#7A6A55]">{m.body}</p>
              <p className="mt-1 text-[10px] text-[#A08D74]">אל: {m.to}</p>
            </div>
          ))}
        </div>
      )}
