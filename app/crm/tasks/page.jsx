...
export default function TasksPage() {
  ...
  return (
    <div className="px-4 py-6">
      <h1 className="text-xl font-bold text-[#3B332A]">משימות</h1>
      <p className="mt-1 text-[13px] text-[#8B8175]">{open.length} משימות פתוחות</p>

      <div className="mt-4 rounded-3xl border border-[#E7DECD] bg-white p-4 shadow-[0_4px_18px_rgba(58,51,53,0.06)]">
        <p className="mb-2 text-[13px] font-semibold text-[#3B332A]">{role === "admin" ? "משימה חדשה - שיוך לצוות" : "משימה חדשה"}</p>
        <div className="space-y-2">
          <input
            ...
            className="w-full rounded-xl border border-[#E7DECD] bg-white px-3 py-2.5 text-sm outline-none focus:border-[#6F4A2E]"
          />
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-xl border border-[#E7DECD] bg-white px-3 py-2.5 text-sm outline-none focus:border-[#6F4A2E]"
            />
            {role === "admin" ? (
              <select
                value={assigneeId}
      ...
      {pushedToMe.length > 0 && (
        <div className="mt-6">
          <p className="mb-2 flex items-center gap-1.5 text-[13px] font-bold text-[#6F4A2E]">
      ...
      <div className="mt-6 space-y-2">
        ...
        {open.length === 0 && <p className="py-6 text-center text-sm text-[#8B8175]">אין משימות פתוחות 🎉</p>}
      </div>

      {done.length > 0 && (
        <div className="mt-6">
          <p className="mb-2 text-[13px] font-semibold text-[#B4AA9C]">הושלמו</p>
          <div className="space-y-2">
            {done.map((t) => (
              <div key={t.id} className="flex items-center gap-3 rounded-2xl border border-[#E7DECD] bg-[#FAF6EE] p-3 opacity-70">
                <button
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-[#3B332A] line-through">{t.title}</p>
                  <p className="text-[11px] text-[#B4AA9C]">
...
function TaskRow({ task, onToggle, highlighted }) {
  ...
  return (
    <div
      className={`rounded-2xl border p-3 ${highlighted ? "border-[#6F4A2E] bg-[#F3E7D5]" : "border-[#E7DECD] bg-white"}`}
    >
      <div className="flex items-center gap-3">
        <button
          onClick={onToggle}
          aria-label="סימון כהושלם"
          className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 border-[#6F4A2E]"
        />
        <div className="flex-1">
          <p className="text-sm font-semibold text-[#3B332A]">{task.title}</p>
      ...
      {candidate && (
        <div className="mt-2 flex items-center justify-between gap-2 rounded-xl bg-white px-3 py-2 text-[12px] shadow-sm">
          <Link href={`/crm?openCandidate=${candidate.id}`} className="flex items-center gap-1.5 font-semibold text-[#6F4A2E]">
          ...
          {candidate.phone && (
            <a
              ...
              dir="ltr"
              className="flex items-center gap-1 rounded-lg bg-[#FAF6EE] px-2 py-1 font-semibold text-[#3B332A]"
            >
