...
//
// כל האוספים כאן נושאים את הקידומת "form_" כי צד השאלון וה-CRM חיים באותו
// פרויקט Firebase, ולשניהם יש מושגים בשם דומה (מועמדים, משימות).
// הקידומת שומרת את הנתונים של שני הצדדים בשני מקומות נפרדים לגמרי.
...
function startSubscriptions() {
  ...
  onSnapshot(doc(db, "form_meta", "config"), (snap) => {
    if (snap.exists()) {
      const d = snap.data();
      ...
      seeded = true;
      setDoc(doc(db, "form_meta", "config"), {
        intro: DEFAULT_INTRO,
        openQuestions: DEFAULT_OPEN_QUESTIONS,
        adminPassword: "admin1234",
      });
      addDoc(collection(db, "form_reps"), {
        name: "נציג לדוגמה",
...
export async function addCandidate(candidate) {
  await ensureAuth();
  return addDoc(collection(db, "form_candidates"), {
...
export async function updateCandidate(id, patch) {
  await ensureAuth();
  return updateDoc(doc(db, "form_candidates", id), patch);
}
export async function deleteCandidate(id) {
  await ensureAuth();
  await deleteDoc(doc(db, "form_candidates", id));
  await Promise.all(
    cache.matches
      .filter((m) => m.manId === id || m.womanId === id)
      .map((m) => deleteDoc(doc(db, "form_matches", m.id)))
  );
  await Promise.all(
    cache.tasks
      .filter((t) => t.candidateId === id)
      .map((t) => deleteDoc(doc(db, "form_tasks", t.id)))
  );
}
...
export async function addRep(rep) {
  await ensureAuth();
  return addDoc(collection(db, "form_reps"), rep);
}
export async function updateRep(id, patch) {
  await ensureAuth();
  return updateDoc(doc(db, "form_reps", id), patch);
}
export async function deleteRep(id) {
  await ensureAuth();
  return deleteDoc(doc(db, "form_reps", id));
}

// ----- משימות -----
export async function addTask(task) {
  await ensureAuth();
  return addDoc(collection(db, "form_tasks"), { done: false, ...task });
}
export async function updateTask(id, patch) {
  await ensureAuth();
  return updateDoc(doc(db, "form_tasks", id), patch);
}
export async function deleteTask(id) {
  await ensureAuth();
  return deleteDoc(doc(db, "form_tasks", id));
}

// ----- התאמות -----
export async function addMatch(match) {
  await ensureAuth();
  return addDoc(collection(db, "form_matches"), {
    status: "נוצרה התאמה",
    createdAt: new Date().toISOString(),
    ...match,
  });
...
export async function updateMatch(id, patch) {
  await ensureAuth();
  return updateDoc(doc(db, "form_matches", id), patch);
}
export async function deleteMatch(id) {
  await ensureAuth();
  return deleteDoc(doc(db, "form_matches", id));
}

// ----- הגדרות (הקדמה, שאלות, סיסמת מנהלת) -----
export async function updateOpenQuestions(questions) {
  await ensureAuth();
  return setDoc(doc(db, "form_meta", "config"), { openQuestions: questions }, { merge: true });
}
export async function updateIntro(intro) {
  await ensureAuth();
  return setDoc(doc(db, "form_meta", "config"), { intro }, { merge: true });
}
export async function updateAdminPassword(password) {
  await ensureAuth();
  return setDoc(doc(db, "form_meta", "config"), { adminPassword: password }, { merge: true });
}
