...
export async function fetchAppSettings(supabase) {
  const { data, error } = await supabase.from("app_settings").select("*").eq("id", 1).single();
  if (error) throw error;
  return data;
}

export async function updateAppSettings(supabase, fields) {
  const { error } = await supabase
    .from("app_settings")
...
export async function acceptTerms(supabase, userId) {
  const { error } = await supabase.from("profiles").update({ terms_accepted: true }).eq("id", userId);
  if (error) throw error;
}
