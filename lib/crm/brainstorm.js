...
export function buildInviteMessage({ candidateName, question, link, days = ROUND_DAYS }) {
  return [
    `נפתח סבב סיעור מוחות חדש על ${candidateName || "מועמד/ת"} 💡`,
    "",
    `השאלה לסבב הזה: ${question}`,
    "",
    `יש לכם ${days} ימים להיכנס, לכתוב ולהגיב.`,
    link ? `כניסה לזירה: ${link}` : "",
  ]
