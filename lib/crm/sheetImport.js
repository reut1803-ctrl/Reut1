...
export function rowToCandidate(row, mapping) {
  const get = (field) => (mapping[field] === undefined ? "" : String(row[mapping[field]] ?? "").trim());

  const gender = detectGender(get("gender")) || "male";
  ...
  const candidate = {
    ...
    religiousLevel: pickOption(get("religiousLevel"), religiousLevelsFor(gender)) || religiousLevelsFor(gender)[0],
    photoUrl: photo,
  ...
  if (gender === "female") {
    candidate.education = pickOption(get("education"), EDUCATION_OPTIONS) || EDUCATION_OPTIONS[0];
    candidate.yeshivaLevel = null;
  } else {
    candidate.yeshivaLevel = pickOption(get("yeshivaLevel"), YESHIVA_LEVELS) || YESHIVA_LEVELS[0];
