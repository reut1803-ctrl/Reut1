...
import { DEFAULT_TERMS_TEXT, DEFAULT_DAILY_TIP, DEFAULT_TAG_GROUPS, candidateTagsByGroup } from "./mockData";
...
export const useCrmStore = create((set, get) => ({
  ...
  initCrmFirebase: () => {
    ...
    onSnapshot(doc(crmDb, "settings", "app"), (d) => {
      ...
      set({
        ...
        tagGroups: data.tagGroups ?? DEFAULT_TAG_GROUPS,
      });
    });
  ...
  recomputeRole: () => {
    ...
    const match = authAllowlist.find((e) => e.email.trim().toLowerCase() === myEmail);
    if (!match) {
      set({ role: "unauthorized", currentStaffEmail: null });
      return;
    }
    set({ role: match.role, currentStaffEmail: match.role === "staff" ? match.email.trim().toLowerCase() : null });
  },
  currentUser: () => {
  ...
  removeAllowlistEntry: async (email) => {
    await deleteDoc(doc(crmDb, "staffAllowlist", email.trim().toLowerCase()));
  },
  ...
  // --- סינון בפיד ---
  filters: { ageRange: [20, 40], heightRange: [150, 195], religiousLevel: "הכל", region: "הכל", search: "", tag: null, tags: {} },
  setFilters: (partial) => set((state) => ({ filters: { ...state.filters, ...partial } })),
  resetFilters: () =>
    set((state) => ({
      filters: { ageRange: [20, 40], heightRange: [150, 195], religiousLevel: "הכל", region: "הכל", search: "", tag: state.filters.tag, tags: state.filters.tags },
    })),
  ...
  // --- קטגוריות תוויות (מצב משפחתי, השתייכות וכו') - נשמרות ב-Firestore וניתנות לעריכה ---
  tagGroups: DEFAULT_TAG_GROUPS,
  saveTagGroups: async (groups) => {
    await setDoc(doc(crmDb, "settings", "app"), { tagGroups: groups }, { merge: true });
  },
  addTagOption: async (groupId, option) => {
    const name = option.trim();
    if (!name) return;
    const next = get().tagGroups.map((g) =>
      g.id === groupId && !g.options.includes(name) ? { ...g, options: [...g.options, name] } : g
    );
    await get().saveTagGroups(next);
  },
  removeTagOption: async (groupId, option) => {
    const next = get().tagGroups.map((g) =>
      g.id === groupId ? { ...g, options: g.options.filter((o) => o !== option) } : g
    );
    await get().saveTagGroups(next);
  },
  addTagGroup: async (label) => {
    const name = label.trim();
    if (!name) return;
    const id = `g${Date.now()}`;
    await get().saveTagGroups([...get().tagGroups, { id, label: name, color: "#8C4A55", options: [] }]);
  },
  removeTagGroup: async (groupId) => {
    await get().saveTagGroups(get().tagGroups.filter((g) => g.id !== groupId));
  },
  // בודק אם מועמד/ת עומד/ת בסינון התוויות: בתוך קטגוריה מספיקה התאמה אחת,
  // ובין קטגוריות שונות נדרשת התאמה בכולן (סינון משולב).
  matchesTagFilters: (candidate) => {
    const { filters, tagGroups } = get();
    const selected = filters.tags || {};
    const byGroup = candidateTagsByGroup(candidate, tagGroups);
    return Object.entries(selected).every(([groupId, chosen]) => {
      if (!chosen || chosen.length === 0) return true;
      const has = byGroup[groupId] || [];
      return chosen.some((c) => has.includes(c));
    });
  },
  toggleTagFilter: (groupId, option) =>
    set((state) => {
      const cur = state.filters.tags?.[groupId] || [];
      const next = cur.includes(option) ? cur.filter((o) => o !== option) : [...cur, option];
      return { filters: { ...state.filters, tags: { ...state.filters.tags, [groupId]: next } } };
    }),
  clearTagFilters: () => set((state) => ({ filters: { ...state.filters, tags: {} } })),
