...
export const useCrmStore = create((set, get) => ({
  ...
  initCrmFirebase: () => {
    ...
    onSnapshot(
      ...
      () => {
        set({ allowlistLoaded: true, allowlistError: true });
        get().recomputeRole();
      }
    );
  ...
  recomputeRole: () => {
    const { googleUser, authAllowlist, allowlistError } = get();
    if (!googleUser) {
    ...
    if (!match) {
      // אם הקריאה עצמה נכשלה איננו יודעים אם יש הרשאה או לא, ולכן לא נועלים בחוץ
      set({ role: allowlistError ? "unverified" : "unauthorized", currentStaffEmail: null });
      get().subscribeTasks();
      return;
    }
  ...
  // הצעה קודמת לאותו זוג שירדה מהפרק - לתזכורת ידידותית לפני הצעה חוזרת
  droppedProposalFor: (maleId, femaleId) =>
    get().proposals.find((p) => p.maleId === maleId && p.femaleId === femaleId && p.status === PROPOSAL_DROPPED),
