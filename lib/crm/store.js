...
export const useCrmStore = create((set, get) => ({
  ...
  initCrmFirebase: () => {
    ...
    onSnapshot(doc(crmDb, "settings", "app"), (d) => {
      ...
      set({
        ...
        dailyTip: data.dailyTip ?? DEFAULT_DAILY_TIP,
  ...
  resetFilters: () =>
    set({ filters: { ageRange: [20, 40], heightRange: [150, 195], religiousLevel: "הכל", region: "הכל", search: "" } }),
  ...
  setDailyTip: async (text) => {
    await setDoc(doc(crmDb, "settings", "app"), { dailyTip: text }, { merge: true });
  },
