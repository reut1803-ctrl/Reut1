...
// אוצר המילים של סטטוס ההצעה יושב במודול נפרד ונטול תלויות
// (lib/crm/proposalStatus.js), ונשמר כאן כייצוא חוזר כדי שכל מי
// שמייבא אותו מהחנות - וזה כל המסכים - ימשיך לעבוד בדיוק כשהיה.
import {
  PROPOSAL_STAGES,
  PROPOSAL_DROPPED,
  PROPOSAL_FROZEN,
...
export { PROPOSAL_STAGES, PROPOSAL_DROPPED, PROPOSAL_FROZEN };
...
export const useCrmStore = create((set, get) => ({
  ...
  // הקפאה ושחרור מהקפאה.
  //
  // הקפאה זוכרת את השלב שבו ההצעה היתה (frozenFrom), והשחרור מחזיר
  // אליו בדיוק - כך שאין "איפוס" של התקדמות שכבר נעשתה. שתי הפעולות
  // נרשמות ביומן ההתקדמות כמו כל שינוי סטטוס אחר, ולכן אפשר תמיד
  // לראות מתי הוקפאה, בידי מי, ומתי חזרה.
  setProposalFrozen: async (proposalId, frozen, note = "") => {
    const proposal = get().proposals.find((p) => p.id === proposalId);
    if (!proposal) return;
    const author = get().currentUser().name;
    const now = new Date().toISOString();
    const status = frozen ? PROPOSAL_FROZEN : restoredStatus(proposal);
    // הקפאה חוזרת של הצעה שכבר מוקפאת לא תדרוס את השלב השמור
    const frozenFrom = frozen ? (isFrozenProposal(proposal) ? proposal.frozenFrom || null : proposal.status) : null;
    const journal = [
      ...proposal.journal,
      {
        id: `j-${Date.now()}`,
        date: now,
        status,
        note: note || (frozen ? "ההצעה הוקפאה" : "ההצעה הוחזרה לפעילות"),
        author,
      },
    ];
    await updateDoc(doc(crmDb, "proposals", proposalId), { status, frozenFrom, journal });
    await Promise.all([get().touchCandidate(proposal.maleId), get().touchCandidate(proposal.femaleId)]);
  },
  ...
  // עדכון פרטיו של אדם שאינו במאגר ("מהמעגל האישי"), השמורים
  // בתוך ההצעה עצמה ולא בכרטיס נפרד.
  //
  // פתוח לכל אנשי הצוות ובכל שלב של ההצעה, בלי חלון זמן ובלי קשר
  // למי פתח/ה אותה. זו הנקודה: תיק עובר בין שדכניות, ומי שמקבל/ת
  // אותו באמצע חייב/ת להשלים פרטים בלי להיתקל בחסימה.
  //
  // side = "male" | "female"
  updateProposalExternal: async (proposalId, side, partial) => {
    const key = side === "female" ? "externalFemale" : "externalMale";
    const proposal = get().proposals.find((p) => p.id === proposalId);
    const current = proposal?.[key] || {};
    const next = stripUndefined({ ...current, ...partial });
    const data = { [key]: next };
    // שינוי שם מעדכן גם את השם שנשמר על ההצעה, כדי שהכותרת
    // והארכיון לא ימשיכו להציג את השם הישן.
    if (typeof partial?.name === "string" && partial.name.trim()) {
      data[side === "female" ? "femaleName" : "maleName"] = partial.name.trim();
    }
    await updateDoc(doc(crmDb, "proposals", proposalId), data);
  },
  ...
  // הלוח הפעיל: בלי מה שירד מהפרק, בלי מה שמוקפא, ובלי היסטוריה ידנית
  activeProposals: () => get().proposals.filter(isActiveProposal),
  ...
  // סימון / ביטול סימון ל"זרקור היומי".
  //
  // חותמת הסימון נכתבת בשרת (serverTimestamp) ולא במכשיר, כמו כל
  // מנגנוני הזמן במערכת. לפיה נקבע סדר הזרקור, ולכן הסדר זהה
  // לכל אישי הצוות ובכל מכשיר, בלי תלות בשעון המקומי.
  //
  // touch: false בכוונה - סימון לזרקור אינו טיפול במועמד/ת.
  setCandidateSpotlight: async (id, on) => {
    await get().updateCandidate(
      id,
      { spotlight: !!on, spotlightAt: on ? serverTimestamp() : null },
      { touch: false }
    );
  },
  ...
  // מחיקת כרטיס לצמיתות.
  //
  // שמור לבעלת המערכת בלבד (OWNER_EMAIL). הבדיקה כאן היא שכבה שנייה:
  // הכפתור כלל אינו מוצג לאחרים, והשרת עצמו חוסם מחיקה למי שאינו מנהלת.
  //
  // מה נמחק: הכרטיס עצמו, מפתחות החיפוש הציבוריות שלו (nameIndex),
  // כרטיס הסטטוס הציבורי, סבבי סיעור המוחות עליו עם הכרטיסיות שבהם,
  // וכל המדיה שלו (הקלטות ו-PDF) - כדי שלא יישאר מידע אישי יתום במסד.
  //
  // מה נשמר במכוון: הצעות השידוך. המערכת בנויה מלכתחילה להציג הצעה
  // שצד אחד שלה נמחק לפי השם שנשמר בה, כדי שהרציונל, היומן והסטטוס
  // לא ייעלמו מהמסך. גם משימות וחיובים נשמרים - הם שייכים לצוות
  // ולהנהלת חשבונות, והמסכים שלהם יודעים להתמודד עם כרטיס שאינו קיים.
  deleteCandidate: async (id) => {
    const me = String(get().googleUser?.email || "").trim().toLowerCase();
    if (!id || me !== OWNER_EMAIL) return false;

    const candidate = get().candidates.find((c) => c.id === id);

    // 1. מדיה: הקלטות הצוות, הקלטת ההיכרות וה-PDF. נמחקת לפני הכרטיס
    //    עצמו, כי ההפניות אליה שמורות על הכרטיס.
    const mediaRefs = [
      candidate?.pdfUrl,
      candidate?.introAudioUrl,
      ...(candidate?.voiceNotes || []).map((n) => n?.audioUrl),
    ].filter(Boolean);
    await Promise.all(mediaRefs.map((ref) => deleteMedia(ref).catch(() => {})));

    // 2. סבבי סיעור מוחות על המועמד/ת, יחד עם הכרטיסיות שבתוכם.
    const rounds = get().brainstormRounds.filter((r) => r.candidateId === id);
    for (const r of rounds) {
      await get().deleteBrainstormRound(r.id).catch(() => {});
    }

    // 3. מפתחות החיפוש הציבוריות. בלי זה, מי שימלא את הטופס החיצוני
    //    באותו שם ישויך לכרטיס שכבר אינו קיים.
    try {
      const snap = await getDocs(query(collection(crmDb, "nameIndex"), where("candidateId", "==", id)));
      await Promise.all(snap.docs.map((d) => deleteDoc(d.ref).catch(() => {})));
    } catch {
      /* אין הרשאה או אין מפתחות - אין בכך כדי לעצור את המחיקה */
    }

    // 4. כרטיס הסטטוס הציבורי (הקישור האישי). קודם מרוקנים אותו
    //    מכל פרט מזהה, ורק אז מנסים למחוק אותו לגמרי. כך גם אם כללי
    //    האבטחה שבשרת עדיין אינם מתירים מחיקה, לא נותר שם חשוף במסד.
    await setDoc(
      doc(crmDb, "candidateStatus", id),
      { name: "", availabilityStatus: "", deleted: true },
      { merge: true }
    ).catch(() => {});
    await deleteDoc(doc(crmDb, "candidateStatus", id)).catch(() => {});

    // 5. הכרטיס עצמו. המאזין החי של Firestore מסיר אותו מהמסכים
    //    אצל כולם מיד, בלי רענון.
