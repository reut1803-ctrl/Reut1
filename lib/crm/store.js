...
import { ROUND_MS } from "./brainstorm";
...
export const useCrmStore = create((set, get) => ({
  ...
  visibleRounds: () => {
    ...
    const list = [...brainstormRounds].sort((a, b) => new Date(b.openedAt || 0) - new Date(a.openedAt || 0));
    if (role === "admin") return list;
    return list.filter((r) => !!get().findCandidateById(r.candidateId));
  },
  ...
  openBrainstormRound: async ({ candidateId, question }) => {
  ...
  addBrainstormNote: async (roundId, text) => {
