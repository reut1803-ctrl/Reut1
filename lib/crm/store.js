...
export const useCrmStore = create(
  persist(
    (set, get) => ({
      ...
      // הוספת משימה יזומה ע"י המנהלת, משויכת ישירות לאיש/אשת צוות, עם אפשרות לקשר מועמד/ת ספציפי/ת
      pushTaskToStaff: (title, dueDate, assigneeId, candidateId = null) => {
      ...
      setWeeklyGoals: (goals) => set((state) => ({ weeklyGoals: { ...state.weeklyGoals, ...goals } })),
      trackProfileView: () => {
      ...
      addServiceType: (service) =>
        set((state) => ({
          serviceTypes: [...state.serviceTypes, { id: `svc-${Date.now()}`, ...service }],
        })),
      removeServiceType: (id) =>
        set((state) => ({ serviceTypes: state.serviceTypes.filter((s) => s.id !== id) })),

      charges: [],
      createCharge: (candidateId, serviceTypeId, staffId) => {
      ...
      updateChargeCandidatePayment: (chargeId, status, proofUrl) =>
        set((state) => ({
          charges: state.charges.map((c) =>
            c.id === chargeId
              ? {
                  ...c,
                  candidatePaymentStatus: status,
                  candidatePaymentProofUrl: proofUrl !== undefined ? proofUrl : c.candidatePaymentProofUrl,
                  candidatePaidAt: status === "שולם" ? new Date().toISOString() : c.candidatePaidAt,
                }
              : c
          ),
        })),
      updateChargeStaffPayout: (chargeId, status, proofUrl) =>
        set((state) => ({
          charges: state.charges.map((c) =>
            c.id === chargeId
              ? {
                  ...c,
                  staffPayoutStatus: status,
                  staffPayoutProofUrl: proofUrl !== undefined ? proofUrl : c.staffPayoutProofUrl,
                  staffPaidAt: status === "שולם" ? new Date().toISOString() : c.staffPaidAt,
                }
              : c
          ),
        })),
