...
import { buildAttentionData } from "./attention";
...
export const useCrmStore = create((set, get) => ({
  ...
  updateCandidate: async (id, partial, { touch = true } = {}) => {
    ...
    await updateDoc(doc(crmDb, "candidates", id), data);
  ...
  intakeSubmissions: [],
  intakeLoaded: false,
  _convertingIntake: false,
  pendingIntakeCount: () => get().intakeSubmissions.filter((x) => x.status !== "converted").length,
  convertPendingIntake: async () => {
    if (get().role !== "admin" || get()._convertingIntake) return 0;
    const pending = get().intakeSubmissions.filter((x) => x.status !== "converted");
    if (pending.length === 0) return 0;
    set({ _convertingIntake: true });
    let created = 0;
    for (const item of pending) {
      try {
        const candidate = await get().addCandidate({
          ...
          // נולד חסוי בכוונה: מי שנרשם/ה בעצמו/ה עדיין לא דיבר/ה עם אף אחד,
          // והמנהלת רוצה לעבור על הכרטיס וליצור קשר ראשוני לפני שהוא נחשף
          // לצוות. האכיפה כאן היא בשרת ולא רק במסך - הצוות אינו יכול למשוך
          // את הכרטיס בשום דרך. הוא נפתח לצוות בלחיצה על "יצרתי קשר".
          confidential: true,
        });
        ...
        // פנייה בודדת שנכשלה לא עוצרת את השאר, והיא תנוסה שוב בכניסה הבאה
      }
    }
    set({ _convertingIntake: false });
    return created;
  },
