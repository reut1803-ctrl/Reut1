...
export const useCrmStore = create((set, get) => ({
  ...
  initCrmFirebase: () => {
    ...
    onAuthStateChanged(crmAuth, (user) => {
      if (user) {
        ...
        set({ googleUser: null, favorites: {}, personalNotes: {} });
      }
      get().recomputeRole();
  ...
  // --- סינון בפיד ---
  filters: { ageRange: [18, 70], heightRange: [140, 210], religiousLevel: "הכל", region: "הכל", search: "" },
  ...
  // שכפול הצעה קיימת ככרטיס חדש במערכת: אותם צדדים (כולל פרטי מועמד/ת
  // מחוץ למאגר) ואותו רציונל, אך כהצעה חדשה בשלב הראשון ועם יומן נקי.
  duplicateProposal: async (source) => {
    if (!source) return null;
    const author = get().currentUser().name;
    const now = new Date().toISOString();
    const copyExternal = (e) =>
      e && e.name
        ? { name: e.name, notes: e.notes || "", audioUrl: e.audioUrl || null }
        : null;
    const proposal = {
      maleId: source.maleId || null,
      femaleId: source.femaleId || null,
      externalMale: copyExternal(source.externalMale),
      externalFemale: copyExternal(source.externalFemale),
      status: PROPOSAL_STAGES[0],
      createdAt: now,
      rationale: source.rationale || "",
      assignee: null,
      journal: [{ id: `j-${Date.now()}`, date: now, status: PROPOSAL_STAGES[0], note: "ההצעה שוכפלה מהצעה קיימת", author }],
    };
    const ref = await addDoc(collection(crmDb, "proposals"), proposal);
    return { id: ref.id, ...proposal };
  },
