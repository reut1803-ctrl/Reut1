...
import { collection, doc, getDoc, getDocs, query, where, orderBy, setDoc, writeBatch } from "firebase/firestore";
...
export async function resolveMediaUrl(value) {
  ...
  const snap = await getDocs(query(collection(crmDb, "mediaChunks"), where("mediaId", "==", id), orderBy("index")));
