...
// קטגוריות תוויות - נשמרות ב-Firestore (settings/app) וניתנות לעריכה מלוח הבקרה,
// כך שאפשר להוסיף קטגוריות ואפשרויות חדשות בעתיד בלי שינוי בקוד.
// לכל מועמד/ת אפשר לסמן כמה אפשרויות בכל קטגוריה במקביל.
export const DEFAULT_TAG_GROUPS = [
  {
    id: "marital",
    label: "מצב משפחתי",
    color: "#6E3540",
    options: ["רווק/ה", "פרק ב'", "גרוש/ה", "אלמן/ה"],
  },
  {
    id: "sector",
    label: "השתייכות / סגנון",
    color: "#8C4A55",
    options: ["תורני", 'חב"ד', "ברסלב", 'דתל"שים ומסורתיים', "חסידי", "ליטאי", "ספרדי"],
  },
];

// גוון לכל אפשרות בתוך קטגוריה - שומר על זהות צבע לקטגוריה עם הבחנה בין האפשרויות
export function tagChipStyle(group, index) {
  const shades = ["#4A1D26", "#6E3540", "#8C4A55", "#A96470", "#C98894", "#E3B3BB", "#F3DCE0"];
  const base = shades[index % shades.length];
  const light = ["#C98894", "#E3B3BB", "#F3DCE0"].includes(base);
  return { backgroundColor: group?.color && index === 0 ? group.color : base, color: light ? "#3A3335" : "#FFFFFF" };
}

// קורא את התוויות של מועמד/ת לפי קטגוריות, כולל תאימות לאחור לשדה tag הבודד הישן
export function candidateTagsByGroup(candidate, tagGroups) {
  const result = {};
  (tagGroups || []).forEach((g) => {
    const fromNew = Array.isArray(candidate?.tags?.[g.id]) ? candidate.tags[g.id] : [];
    const legacy = candidate?.tag && g.options?.includes(candidate.tag) && !fromNew.includes(candidate.tag) ? [candidate.tag] : [];
    result[g.id] = [...fromNew, ...legacy];
  });
  return result;
}
