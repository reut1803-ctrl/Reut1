// קבועי בחירה (רשימות בחירה בטפסים) וברירות מחדל טקסטואליות בלבד.
// נתוני מועמדים, צוות, משימות והצעות חיים כעת ב-Firestore (ראו lib/crm/firebaseClient.js ו-lib/crm/store.js).

export const REGIONS = ["ירושלים", "מרכז", "שפלה", "שרון", "בנימין", "יהודה", "שומרון", "דרום", "צפון"];

export const RELIGIOUS_LEVELS_MALE = ["חרד\"ל", "חסידי", "תורני", "דתי", "מסורתי", "דתל\"ש", "הכל"];
export const RELIGIOUS_LEVELS_FEMALE = ["חרד\"לית", "חסידית", "תורנית", "דתייה", "מסורתית", "דתל\"שית", "הכל"];
export const religiousLevelsFor = (gender) => (gender === "female" ? RELIGIOUS_LEVELS_FEMALE : RELIGIOUS_LEVELS_MALE);

// מסלול השכלה/עיסוק - למאגר הבנות בלבד (לבנים יש YESHIVA_LEVELS נפרד)
// נשמרים לתאימות לאחור בלבד: כרטיסים ותיקים עדיין מחזיקים את השדות האלה.
export const EDUCATION_OPTIONS = ["תיכון", "אולפנה", "מדרשה", "שירות", "סמינר", "תואר ראשון", "תואר שני", "עובדת ולומדת", "עובדת", "לא משנה"];
export const YESHIVA_LEVELS = ["ישיבה גבוהה", "הסדר", "צבא", "עובד ולומד", "עובד", "תואר ראשון", "תואר שני", "תעודה", "לא משנה"];

// "המסלול שלי" - בחירה מרובה של כל הדרכים שהאדם עבר בחיים.
// נפרד לגמרי מ"עיסוק נוכחי", שהוא טקסט חופשי של מה שהוא/היא עושים היום.
// אפשרויות שמוצגות רק במאגר אחד. הערכים עצמם זהים לאלה שנשמרו מאז ומעולם -
// כאן רק מחליטים מה מוצג, ולכן שום כרטיס קיים לא משנה את הנתונים שלו.
export const MALE_ONLY_OCCUPATIONS = ["ישיבה גבוהה", "ישיבת הסדר", "מכינה"];
export const FEMALE_ONLY_OCCUPATIONS = ["מדרשה", "סמינר", "שירות לאומי"];

export const OCCUPATION_OPTIONS = [
  "ישיבה גבוהה",
  "ישיבת הסדר",
  "מכינה",
  "צבא",
  "שירות לאומי",
  "מדרשה",
  "סמינר",
  "עבודה",
  "עובד/ת ולומד/ת",
  "תואר ראשון",
  "תואר שני",
  "תעודה מקצועית",
  "לימודי הוראה",
  "עסק עצמאי",
];

// רשימת העיסוקים המתאימה למאגר הנצפה: בנים אינם רואים מדרשה או סמינר,
// ובנות אינן רואות ישיבה גבוהה, הסדר או מכינה.
//
// selected = מה שכבר מסומן בכרטיס. ערך שנבחר בעבר תמיד ימשיך להופיע, גם אם
// הוא אינו שייך למאגר הזה - אחרת עריכת כרטיס ותיק הייתה מוחקת אותו בשקט.
export function occupationsFor(gender, selected = []) {
  const hidden = gender === "female" ? MALE_ONLY_OCCUPATIONS : FEMALE_ONLY_OCCUPATIONS;
  const chosen = new Set(selected || []);
  const list = OCCUPATION_OPTIONS.filter((o) => !hidden.includes(o) || chosen.has(o));
  // ערך ישן שאינו ברשימה הכללית כלל - נשמר גם הוא, בסוף
  const extras = [...chosen].filter((o) => !OCCUPATION_OPTIONS.includes(o));
  return [...list, ...extras];
}

// כרטיסים ותיקים נשמרו עם מסלול יחיד (רמת לימוד / השכלה). כאן ממירים אותם
// לרשימת עיסוקים, כדי שהתצוגה והתאמות יעבדו גם עליהם בלי לגעת בנתונים.
export function candidateOccupations(candidate) {
  if (Array.isArray(candidate?.occupations) && candidate.occupations.length > 0) return candidate.occupations;
  const legacy = [candidate?.yeshivaLevel, candidate?.education].filter((v) => v && v !== "לא משנה");
  return [...new Set(legacy)];
}

export const SMOKING_OPTIONS_MALE = ["לא מעשן - חובה", "לא מפריע לי", "לא משנה"];
export const SMOKING_OPTIONS_FEMALE = ["לא מעשנת - חובה", "לא מפריע לי", "לא משנה"];
export const smokingOptionsFor = (gender) => (gender === "female" ? SMOKING_OPTIONS_FEMALE : SMOKING_OPTIONS_MALE);

export const TRAITS = ["הומור", "רוגע", "משפחתיות", "רגישות", "שאפתנות", "רוחניות", "ביטחון עצמי", "מנהיגות"];

// תוויות סינון למאגר המועמדים - סקלת גוונים אחת ממשפחת הצבע הראשי של המערכת, מהכהה לבהיר
export const CANDIDATE_TAGS = [
  { name: "תורני", color: "#4A1D26", textColor: "#FFFFFF" },
  { name: 'דתל"שים ומסורתיים', color: "#6E3540", textColor: "#FFFFFF" },
  { name: 'חב"ד', color: "#8C4A55", textColor: "#FFFFFF" },
  { name: "חוזר/ת בתשובה", color: "#A96470", textColor: "#FFFFFF" },
  { name: "ברסלב", color: "#C98894", textColor: "#3A3335" },
  { name: "פרק ב'", color: "#F3DCE0", textColor: "#3A3335" },
];

// תווית מאוחדת אחת לחזרה בתשובה. כרטיסים ישנים שנשמרו בניסוח אחר
// ("חוזר בתשובה", "חוזרת בתשובה" וכדומה) מוצגים ומסוננים תחת התווית המאוחדת.
export const RETURNEE_TAG = "חוזר/ת בתשובה";

export function normalizeTagName(tag) {
  if (!tag) return tag;
  return tag.includes("בתשובה") ? RETURNEE_TAG : tag;
}

export const DEFAULT_TERMS_TEXT = `נהלי עבודה וסודיות - צוות השידוכים

1. כל מידע אישי על מועמדים ומועמדות (שמות, טלפונים, תמונות, הקלטות, תעודות זהות) חסוי ואסור בשיתוף מחוץ לצוות.
2. אין להעביר פרטי קשר של מועמד/ת לצד שלישי ללא אישור מפורש מהמנהלת.
3. יש לעדכן סטטוס ותיעוד בכרטיס המועמד/ת באופן שוטף ומיידי לאחר כל שיחה או פגישה.
4. פנייה למועמדים תיעשה בשפה מכבדת ורגישה בכל שלב בתהליך.

באישור התיבה למטה, הנך מאשר/ת שקראת את הנהלים ומתחייב/ת לפעול לפיהם.`;

export const DEFAULT_DAILY_TIP =
  "טיפ השבוע: בשיחה ראשונה עם מועמד/ת חדש/ה, התחילו בשאלות פתוחות על התחביבים והיומיום שלהם לפני שעוברים לשאלות על ציפיות מבן/בת הזוג - זה בונה אמון ומוציא תשובות אמיתיות יותר.";

export const DEFAULT_SERVICE_TYPES = [
  { name: "שיחת סינון", price: 150, commission: 40 },
  { name: "מסלול עומק", price: 600, commission: 150 },
];

export const PAYMENT_STATUSES = ["ממתין לתשלום", "שולם"];
