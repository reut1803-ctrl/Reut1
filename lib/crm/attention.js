...
import { isFrozenProposal } from "./proposalStatus";
...
export function buildAttentionData({ proposals = [], rounds = [], notes = [], droppedStatus = "" } = {}) {
  ...
  proposals.forEach((p) => {
    ...
    // הצעה פעילה = כל מה שלא ירד מהפרק, אינו מוקפא, ואינו רשומת
    // היסטוריה ישנה. הצעה מוקפאת אינה "תופסת" את המועמד/ת: הוא/היא
    // נשאר/ת פנוי/ה להצעות חדשות מכל הצוות, וזו כל מטרת ההשהיה.
    if (p.status !== droppedStatus && !isFrozenProposal(p) && !p.isHistory) {
...
// סטטוס שבו המועמד/ת אינו/ה ברשימת הפניות כלל.
export const UNAVAILABLE_STATUS = "לא פנוי";

// כרטיס שאינו פנוי אינו ממתין לטיפול של איש - אין מה לעשות איתו כרגע.
// בודק גם בנפרד מהסטטוס הציבורי וגם מהשדה שעל הכרטיס עצמו,
// כדי שההתנהה תעבוד גם כשהפונקציה מקבלת כרטיס גולמי שלא עבר מיזוג.
export function isUnavailable(candidate, statusMap) {
  const live = statusMap ? statusMap[candidate?.id] : null;
  return (live || candidate?.availabilityStatus) === UNAVAILABLE_STATUS;
}

// חיווי "דורש התייחסות" במסך הפרופילים.
//
// שני מצבים מבטלים את החיווי לגמרי:
// 1. התאמה פעילה - יש תהליך פתוח, וגם אם הוא מתעכב הדיווח עליו
//    שייך למסך השידוכים ולא לכרטיס במאגר.
// 2. סטטוס "לא פנוי" - כרטיס שאינו פנוי להצעות אינו יכול באותה
//    נשימה לדרוש טיפול. שני החיוויים יחד סתרו זה את זה על המסך.
export function needsAttention(candidate, now, data, statusMap) {
  if (isUnavailable(candidate, statusMap)) return false;
...
// בחירת כרטיסי הזרקור.
//
// הזרקור הוא אזור מדויק וממוקד, ולא הצעה אוטומטית: נכנסים אליו
// אך ורק מי שהמנהלת סימנה במפורש. אין בחירה אוטומטית, ולכן אין
// כרטיסים ש"מתחלפים" מעצמם מיום ליום ואין תוצאות רופפות.
//
// המספר מוגבל בקשיחות. אם סומנו יותר מהמכסה, מוצגים האחרונים
// שסומנו - כך שסימון חדש תמיד נכנס למסך מיד.
// הסדר נקבע לפי חותמת הזמן של השרת (spotlightAt) ולא לפי שעון המכשיר.
//
// סינון ההרשאות נשמר במעלה הזרם: הרשימה שמגיעה לכאן כבר נוקתה
// מכרטיסים חסויים וחסומים לפי תפקיד הצופה.
export function pickSpotlight(list, now, data, { max = SPOTLIGHT_MAX } = {}) {
  return (list || [])
    .filter((c) => c.spotlight === true)
    // מיון יציב: הסימון האחרון ראשון. כרטיס שסומן לפני שהחותמת
    // נוספה למערכת נשאר בסוף, ושומר על סדר קבוע בין שווים.
    .sort((a, b) => toMillis(b.spotlightAt) - toMillis(a.spotlightAt))
    .slice(0, max);
}
...
// הצעה שירדה מהפרק ממשיכה להופיע בכרטיס המועמד/ת במשך יומיים, ואז נעלמת
// מהמסך כדי שהכרטיס יישאר נקי.
//
// חשוב: זו הסתרה בתצוגה בלבד. ההצעה עצמה נשמרת במסד הנתונים לתמיד,
// ולכן התראת הכפילות ("הצעה בין מועמדים אלו עלתה בעבר וירדה מהפרק")
// ממשיכה לעבוד גם שנים אחר כך. מי שקורא את הקוד הזה: אין למחוק כאן דבר.
export const DROPPED_VISIBLE_MS = 48 * 60 * 60 * 1000;
...
// האם להציג את ההצעה בכרטיס המועמד/ת.
// כל מה שלא ירד מהפרק מוצג תמיד. מה שירד - רק ביומיים הראשונים.
//
// הצעה מוקפאת אינה מוצגת כאן כלל: המועמד/ת צריך/ה להיראות פנוי/ה
// לשאר הצוות, ולכן אין לה מקום ב"התקדמות בהתאמות" שבכרטיס. היא
// ממתינה באזור המוקפאים שבמסך ההצעות ואינה הולכת לאיבוד.
export function isProposalRowVisible(proposal, now, droppedStatus) {
  if (isFrozenProposal(proposal)) return false;
...
export function droppedHoursLeft(proposal, now, droppedStatus) {
  const ms = droppedAtMs(proposal, droppedStatus);
  if (!ms) return 0;
  return Math.max(0, Math.ceil((ms + DROPPED_VISIBLE_MS - now) / (60 * 60 * 1000)));
}
