...
export function pickSpotlight(list, now, data, { max = SPOTLIGHT_MAX } = {}) {
  // סימון ידני מוצג במלואו ואינו מוגבל במספר: הזרקור הוא אזור שמרכז את כל
  // מי שסומן/ה, מכל הקהילות והסגנונות, בדיוק כמו המועדפים.
  // המגבלה של ארבעה נוגעת אך ורק לבחירה האוטומטית שלמטה.
