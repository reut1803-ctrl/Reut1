...
const PHONE_RE_G = /0\d{1,2}[-.\s]?\d{3}[-.\s]?\d{3,4}/g;
...
// ניחוש שם איש קשר ממשפט: מסירים טלפונים, מילות מפתח וסימנים, ומשאירים 2-4 מילים ראשונות.
function guessContactName(sentence) {
  return sentence
    .replace(PHONE_RE_G, " ")
    .replace(/איש קשר|אנשי קשר|לבירורים|בירורים|ממליצ(?:ה|ים)?|ממליץ|טלפון|נייד|פלאפון|מספר|של|אצל|קשר/g, " ")
    .replace(/[^א-ת\s]/g, " ")
    .replace(/\s+/g, " ")
    .trim()
    .split(" ")
...
export function parseCandidateText(rawText, openQuestions = []) {
  ...
  // כל הטלפונים בטקסט: הראשון = טלפון המועמד/ת; הנוספים = אנשי קשר לבירורים.
  const allPhones = [...new Set((text.match(PHONE_RE_G) || []).map(cleanPhone).filter(Boolean))];
  if (!fields.phone && allPhones[0]) fields.phone = allPhones[0];
  const references = [];
  ...
  rawLines.forEach((line, idx) => {
    ...
    for (const sentence of splitSentences(t)) {
      // אנשי קשר / מספרים לבירורים: טלפונים נוספים (מעבר לטלפון המועמד/ת) או ציון "איש קשר".
      const phonesIn = [...new Set((sentence.match(PHONE_RE_G) || []).map(cleanPhone))];
      const extraPhones = phonesIn.filter((p) => p && p !== fields.phone && !references.some((r) => r.phone === p));
      const isContact = /איש קשר|אנשי קשר|לבירורים|בירורים|ממליצ|ממליץ|ערבה|ערב\b/.test(sentence);
      if (extraPhones.length || (isContact && phonesIn.length === 0)) {
        const name = guessContactName(sentence);
        if (extraPhones.length) {
          extraPhones.forEach((p, i) => references.push({ name: i === 0 ? name : "", relation: "", phone: p }));
        } else {
          references.push({ name, relation: "", phone: "" });
        }
        anyUsed = true;
        continue;
      }
      if (phonesIn.includes(fields.phone)) { anyUsed = true; continue; } // משפט של טלפון המועמד/ת
  ...
  return { fields, answers, references, description: leftover.join("\n").trim() };
}
