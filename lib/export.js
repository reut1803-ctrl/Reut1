...
export async function downloadPdf(candidate, openQuestions) {
  ...
  html += `<div style="margin-bottom:24px;color:#888;">${g === "female" ? "בחורה" : "בחור"}</div>`;
  PERSONAL_FIELDS.forEach((f) => {
    html += `<div style="margin-bottom:6px;"><strong>${esc(genderLabel(f, g))}:</strong> ${esc(candidate[f.key])}</div>`;
  });
  html += `<hr style="margin:20px 0;border:none;border-top:1px solid #eee;"/>`;
  (openQuestions || []).forEach((q) => {
    html += `<div style="margin-bottom:14px;"><div style="font-weight:600;color:#A84F4F;">${esc(genderLabel(q, g))}</div><div>${esc(candidate.answers?.[q.key])}</div></div>`;
  });
  if (candidate.references?.length) {
    html += `<hr style="margin:20px 0;border:none;border-top:1px solid #eee;"/>`;
