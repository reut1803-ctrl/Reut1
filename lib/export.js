...
export async function downloadPdf(candidate, openQuestions, canSeeSensitive = false) {
  ...
  const g = candidate.gender;
  let html = `<h1 style="color:#5E2F2D;margin:0 0 8px;font-size:46px;">${esc(candidate.fullName)}</h1>`;
  ...
  PERSONAL_FIELDS.forEach((f) => {
    ...
    html += `<div style="background:#FBF4F2;border-radius:14px;padding:12px 18px;font-size:23px;min-width:45%;"><strong style="color:#5E2F2D;">${esc(genderLabel(f, g))}:</strong> ${esc(candidate[f.key])}</div>`;
  });
  html += `</div>`;
  ...
  (openQuestions || []).forEach((q) => {
    ...
    html += `<div style="font-weight:700;color:#5E2F2D;font-size:25px;margin-bottom:8px;">${esc(genderLabel(q, g))}</div>`;
    html += `<div style="background:#E8DCCB;border-radius:16px;padding:16px 20px;font-size:24px;white-space:pre-wrap;">${esc(candidate.answers?.[q.key])}</div>`;
    html += `</div>`;
  });

  if (candidate.references?.length) {
    html += `<div style="font-weight:700;color:#5E2F2D;font-size:25px;margin:24px 0 8px;">אנשי קשר</div>`;
    candidate.references.forEach((r, i) => {
